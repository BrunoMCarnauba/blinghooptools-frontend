                ...
                <form>
                    <fieldset>
                        ...
                        <div className="input-group">
                            ...
                            <input id="dataInicial" type="date" size={10} value={dataInicial} onChange={(event) => setDataInicial(event.target.value)} />
                        </div>

                        <div className="input-group">
                            <label htmlFor="dataFinal">Data final</label>
                            <input id="dataFinal" type="date" size={10} value={dataInicial} onChange={(event) => setDataFinal(event.target.value)} />
                        </div>
                    </fieldset>
                    ...
                    <fieldset id="opcoes-atualizacao">
                        ...
                        <div className="input-group-horizontal">
                            <input id="enviarClienteComCodigo" type="checkbox" checked={enviarClienteComCodigo} onChange={(event) => {setEnviarClienteComCodigo(!enviarClienteComCodigo)}} />
                            <label htmlFor="enviarClienteComCodigo">Enviar cliente com o código</label>
                        </div>

                        <div className="input-group-horizontal">
                            <input id="enviarTodasSituacoesPedido" type="checkbox" checked={enviarTodasSituacoesPedido} onChange={(event) => {setEnviarTodasSituacoesPedido(!enviarTodasSituacoesPedido)}} />
                            <label htmlFor="enviarTodasSituacoesPedido">Enviar pedidos em qualquer situação</label>
                        </div>
                    </fieldset>

                    <div id="botoes-opcoes">
                        <button type="button" onClick={() => migrarPedidos()} disabled={!(dataInicial.length > 0 && dataFinal.length > 0 && loadingStatus=="")}>Migrar pedidos</button>
                    </div>
                </form>

                <Modal titulo="Ajuda" visivel={visualizarAjuda} aoPressionarFechar={() => setVisualizarAjuda(false)}>
                  <div id="modal-ajuda">
                      <p>Adicione a data inicial e a data final em que foram feitos os pedidos que deseja migrar para o Bling.</p>
                      <p>A API do Tiny tem um limite de requisições por minuto que deve ser respeitado, ou seja, a quantidade de pedidos do período de tempo informado não pode ser maior que a quantidade de requisições disponibilizadas.</p>
                      <p>Confira no link a seguir a quantidade correspondente ao seu plano contratado: https://www.bling.com.br/b/preferencias.php#cadastro/cadastros-configuracoes-contatos.</p>
